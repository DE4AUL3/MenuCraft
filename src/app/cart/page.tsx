...
export default function CartPage() {
  ...
  <div className="min-h-screen flex items-center justify-center bg-linear-to-br from-slate-50 via-blue-50 to-indigo-50">
        <div className="text-center max-w-md mx-auto px-6">
          <div className="w-24 h-24 rounded-full mx-auto mb-6 flex items-center justify-center bg-slate-200">
          ...
          <button
            ...
            className="bg-linear-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white py-3 px-6 rounded-xl font-semibold transition-all duration-300 shadow-lg hover:shadow-xl"
          >
            Перейти к меню
          </button>
        </div>
  ...
  <div className="min-h-screen bg-linear-to-br from-slate-50 via-blue-50 to-indigo-50">
      {/* Header */}
      <header className="sticky top-0 z-40 backdrop-blur-lg border-b bg-white/95 border-slate-200/60">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center space-x-4">
              <button
                onClick={() => router.back()}
                className="p-2 transition-colors duration-200 text-slate-600 hover:text-slate-900"
              >
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="space-y-4 mb-8">
          {cartState.items.map((item) => (
            <div
              key={item.id}
              className="rounded-2xl shadow-lg border p-6 bg-white border-slate-200/60"
            >
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  ...
                  <div className="flex items-center space-x-4">
                    <div className="flex items-center space-x-2 rounded-lg p-2 bg-slate-50">
                      <button
                        onClick={() => updateQuantity(item.id, item.quantity - 1)}
                        className="w-6 h-6 rounded-full flex items-center justify-center transition-colors bg-slate-200 hover:bg-slate-300 text-slate-600"
                      >
                        <Minus className="w-3 h-3" />
                      </button>
                      <span className="font-semibold min-w-8 text-center text-slate-900">
                        {item.quantity}
                      </span>
                      <button
                        onClick={() => updateQuantity(item.id, item.quantity + 1)}
        ...
        {/* Order Summary */}
        <div className="rounded-2xl shadow-lg border p-6 mb-6 bg-white border-slate-200/60">
